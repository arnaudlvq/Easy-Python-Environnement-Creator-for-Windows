...
REM   7. **NEW** – If a shortcut is created, also generate a folder named
REM      "path" **inside the virtual‑environment directory** containing a file
REM      (no extension) whose name matches the shortcut.
REM ----------------------------------------------------------------------------
...
REM ---------------------------------------------------------------------------
REM NEW FEATURE: create a "path" directory in the *virtual‑environment* folder
REM              containing an empty file named after the shortcut
REM ---------------------------------------------------------------------------
set "PATH_DIR=%VENV_PATH%\path"
if not exist "%PATH_DIR%" (
    mkdir "%PATH_DIR%"
)
> "%PATH_DIR%\%CMDNAME%" echo(
